...
export class ConversationService {
  ...
  private async checkEmbeddingsExist(emailId: string): Promise<boolean> {
    const result = await this.prisma.primary.$queryRaw<{ has_embedding: boolean }[]>`
...
//   }
private async detectTopic(message: IMessage, department: string): Promise<ConversationTopic> {
    try {
      // Initial keyword-based checks for quick classification
      const subject = (message.subject || '').toLowerCase();
      const body = this.htmlToText(message.body).toLowerCase();
      
      if (subject.includes('interview') || body.includes('interview schedule')) {
        return 'INTERVIEW';
      }
      if (subject.includes('contract') || body.includes('offer letter')) {
        return 'CONTRACT';
      }
      if (subject.includes('resume') || subject.includes('cv') || body.includes('candidate profile')) {
      ...
      const companyInfo = companyRelation ? await this.prisma.primary.company.findUnique({
      ...
      try {
        ...
        const predictedTopic = response.choices[0].message.content?.trim() as ConversationTopic;
...
//   }
  ...
  private getDepartment(metaData: any, emailData: IEmailParticipants): string {
    ...
    if (emailData.recipients) {
        emailData.recipients.forEach(email => allEmails.add(email.toLowerCase()));
    }
    if (emailData.cc_recipients) {
        emailData.cc_recipients.forEach(email => allEmails.add(email.toLowerCase()));
    }
    if (emailData.bcc_recipients) {
        emailData.bcc_recipients.forEach(email => allEmails.add(email.toLowerCase()));
    }
    ...
    if (hasGenericEmail) {
        this.logger.log("Generic email found in participants, setting department to OTHER");
...
private hasMatchingParticipants(participants: Set<string>, categoryEmails: Set<string>): boolean {
...
private getAllParticipants(emailData: IEmailParticipants): Set<string> {
  ...
  private addEmailsToSet(set: Set<string>, emails: string[] | null | undefined): void {
