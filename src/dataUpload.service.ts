...
import { ALL_TOOLS, prompt } from './tools';
...
export class EmailService {
  ...
  private async setUpValidationGraph() {
    ...
    const model = this.chatOpenAi.bindTools(ALL_TOOLS);

    const callModel = async (state: typeof this.GraphAnnotation.State) => {
      const messages = state.messages;
      const response = await model.invoke(messages);

      // If this is a tool call response, just pass it through
      ...
      // Extract and collect tool responses
      ...
      this.logger.log('Validated Classifications:', validatedClassifications);
